...
<body class=" bg-danger bg-gradient ">
    ...
    <div class="container-fluid ">
        <form class="p-5" action="" method="POST">
            <h2 class="text-center text-white">Vender Produto 🎸</h2>
            ...
            <div class="mb-3 ">
                <label for="" class="form-label text-white">Quantidade Para Vender</label>
                <input value="<?php echo $value ?>" name="qtdVenda" class="form-control" type=" <?php
                                                                                                if ($sal_row['qtdestoque'] > 0)
                                                                                                    echo "number";
                                                                                                elseif ($sal_row['qtdestoque'] == 0)
                                                                                                    echo "text";

                                                                                                ?>" required <?php if ($sal_row['qtdestoque'] > 0)
                                                                                                                echo "";
                                                                                                            elseif ($sal_row['qtdestoque'] == 0)
                                                                                                                echo "disabled"; ?>>
            </div>

            <input class="btn btn-success" name="enviar" type="submit" value="Vender Produto" <?php if ($sal_row['qtdestoque'] > 0)
                                                                                                    echo "";
                                                                                                elseif ($sal_row['qtdestoque'] == 0)
                                                                                                    echo "disabled"; ?>>
        ...
        if (isset($_POST['enviar'])) {
            $sal_qtdVenda = $_POST['qtdVenda'];

            if ($sal_qtdVenda > $sal_row['qtdestoque']) {
                echo
                '<script type="text/javascript">
                    var mensagem = "Estoque Não Suficiente"; 
            ...
            } elseif ($sal_qtdVenda <= $sal_row['qtdestoque'] && $sal_qtdVenda > 0) {


                $sal_sql = "UPDATE instrumentos SET qtdestoque = qtdestoque - $sal_qtdVenda WHERE codigo = '$sal_cod'";
                ...
                if ($sal_result) {
                    echo
                    '<script type="text/javascript">
                    var mensagem = "Venda Concluida Com Sucesso!"; 
                ...
                echo
                '<script type="text/javascript">
                    var mensagem = "Impossivel Vender Um Valor Negativo de Unidades!"; 
